...
function createCronJob(){
    ...
    cron.schedule('0 * * * *', async () => {
        ...
        const bloodSugarCsv = user.bloodSugar.map(data => `${data.date},${data.value}`).join('\n');
        ...
        s3.upload(stepCountParams, (err, data) => {
            ...
            console.log('stepCount file uploaded successfully:', data.Location);
            }
        });
        ...
        s3.upload(bloodSugarParams, (err, data) => {
            ...
            console.log('bloodSugar file uploaded successfully:', data.Location);
            }
        });
    }});
}
